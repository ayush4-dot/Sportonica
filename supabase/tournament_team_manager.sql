-- ================================================================
-- TEAM MANAGER — a public point of contact for each team, collected
-- at registration (both self-service and admin-created walk-in
-- teams). Distinct from the team's players, whose own phone/email
-- stay private (see get_team_roster_public()) — a manager's name and
-- phone are meant to be visible to everyone browsing the tournament,
-- so opposing teams/organizers/spectators have someone to reach.
...
-- ── register_team: now requires a manager name + phone ──────────
drop function if exists public.register_team(uuid,text,boolean);
create or replace function public.register_team(
  p_tournament_id uuid, p_name text, p_ack_terms boolean, p_manager_name text, p_manager_phone text
)
...
declare
  ...
  v_manager_name text := trim(coalesce(p_manager_name, ''));
  v_manager_phone text := trim(coalesce(p_manager_phone, ''));
...
begin
  ...
  if v_manager_name = '' then raise exception 'MANAGER_NAME_REQUIRED'; end if;
  if v_manager_phone = '' then raise exception 'MANAGER_PHONE_REQUIRED'; end if;
...
-- ── create_walkin_team: same requirement for a manager name + phone ──
