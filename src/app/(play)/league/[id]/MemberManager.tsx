...
function Avatar({ name, url }: { name: string; url: string | null }) {
  ...
  return (
    <div style={{ width: 38, height: 38, borderRadius: "50%", overflow: "hidden", background: "linear-gradient(150deg,#DE3163,#FFC93C)", display: "grid", placeItems: "center", fontWeight: 800, color: "#0B0D11", flexShrink: 0 }}>
          ...
          {q.trim().length < 2 ? (
            ...
            <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 11, padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <button onClick={() => invite(p.id)} disabled={pending || added.has(p.id)}
                style={{ background: added.has(p.id) ? "transparent" : "#FFC93C", color: added.has(p.id) ? "#2E7D5B" : "#0B0D11", border: added.has(p.id) ? "1px solid rgba(46,125,91,0.4)" : "none", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
