...
export default function DownloadButton({ username, name }: { username: string; name: string }) {
  ...
  async function download() {
    ...
    try {
      ...
      const file = new File([blob], `${username}-sportonica.png`, { type: "image/png" });
      ...
      if (navigator.canShare?.({ files: [file] })) {
        try {
          await navigator.share({ files: [file], title: `${name} · Sportonica` });
        } catch {
      ...
      a.href = url;
      a.download = `${username}-sportonica.png`;
