...
const COLS: Column<PayoutRow>[] = [
  ...
  { key: "commission_total", label: "Our cut", type: "custom", render: (v) => <span className="dt-mono" style={{ color: "#006241" }}>Rs {Number(v.commission_total || 0).toLocaleString("en-IN")}</span> },
  { key: "payout_total", label: "Venue earns", type: "money" },
  { key: "payout_pending", label: "Unpaid", type: "custom", render: (v) => <span className="dt-mono" style={{ color: Number(v.payout_pending) > 0 ? "#d97706" : undefined }}>Rs {Number(v.payout_pending || 0).toLocaleString("en-IN")}</span> },
];
