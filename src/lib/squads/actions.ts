...
export async function joinSquad(squadId: string) {
  ...
  if (sq && sq.cap && sq.member_count >= sq.cap) throw new Error("SQUAD_FULL");
...
export async function removeMember(squadId: string, userId: string) {
  ...
  if (!sq || sq.creator_id !== user.id) throw new Error("FORBIDDEN");
  if (userId === user.id) throw new Error("Can't remove yourself — leave the squad instead.");
...
export async function searchPlayers(q: string, squadId: string) {
